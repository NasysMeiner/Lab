...
void CopyDigit(int allLenght, char* timeLine, char digit) 
{
    timeLine = (char*)realloc(timeLine, (allLenght + 1) * sizeof(char));
    timeLine[allLenght - 1] = digit;
            ...
            if (*currentLenghtWord - *resultLenghtWord >= lenghtWord)
            {
                ...
                if (matches == lenghtWord)
                {
                    ...
                    *resultLenghtWord = *currentLenghtWord;
                    char digit[2];
                    sprintf(digit, "%d", i);
                    CopyDigit(*currentLenghtWord, word, digit[0]);
